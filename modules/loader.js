...
module.exports = {
    ...
    startModules: function (modules,isbeta) {
        ...
        for(m in modules){
            ...
            for(command in moduleCommands){
                ...
                commandList[curmodule][command]['function'] = 'mod[\''+curmodule+'\'].'+commandList[curmodule][command]['function'];
            }
        }
        ...
        for(m in commandList){
            for(c in commandList[m]){
                if(typeof(commandList[m][c].hide)=='undefined' || commandList[m][c].hide===false){
                    if(typeof(commandList[m][c].argument)!=='undefined') start+="/"+c+" "+commandList[m][c].argument+"\n";
                    else start+="/"+c+"\n";
                ...
                bot.command([c, c+'@'+this.username], async (ctx) => {
                    ...
                    if(typeof(commandFunctions[command])!=='undefined') eval('this.'+commandFunctions[command].replace('ARG','"'+arg+'"'));
        ...
        bot.command(['start','start@'+this.username,'help','help@'+this.username], async (ctx) => {
            var mes = ctx.replyWithHTML(start);
        });

        bot.on('new_chat_members', async (ctx) => {

            if(typeof(this.groupWelcome)!=='undefined' && this.groupWelcome!=="") {
                for(m in ctx.update.message.new_chat_members){
                    var member = ctx.update.message.new_chat_members[m];
                    var welcome = this.groupWelcome.replace('[USERNAME]', member.first_name);
    ...
    sendMessage: async function (ctx,message,type,deleteprevious,deletecommand,showpreview,replyto,buttons,image,video){
        ...
        if(ctx.update.message.chat.type!=='private'){
            if(deletecommand) ctx.tg.deleteMessage(chatId, ctx.update.message.message_id);
            if(deleteprevious && typeof(lastMessages[chatId][type])!=='undefined' && lastMessages[chatId][type]>0){
                ctx.tg.deleteMessage(chatId, lastMessages[chatId][type]);
            }
        }

        var rep=replyto;
        if(typeof(ctx.update.message.reply_to_message)!=='undefined') rep=ctx.update.message.reply_to_message.message_id;

        if(image!=='') {
            var mes = ctx.replyWithPhoto({ source: image }, {caption: message ,parse_mode: 'HTML',reply_to_message_id:rep }).then(function(data){
                lastMessages[chatId][type]=data.message_id;
            });

        }
        else if(video!=='') {
            var mes = ctx.replyWithVideo({ source: video }, {caption: message ,parse_mode: 'HTML',reply_to_message_id:rep }).then(function(data){
                lastMessages[chatId][type]=data.message_id;
            });

        }
        else {
            var mes = ctx.replyWithHTML(message, Extra.inReplyTo(rep).webPreview(showpreview).HTML().markup((m) => {
                let list = []
