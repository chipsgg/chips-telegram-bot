  ...
  .then(() => {
    ...
    bot.command("divs", (ctx) => {
      ...
      const currencies = _.chain(API.get("public", "currencies"))
        .filter(
          (x) => !x.hidden && x.name !== "chips" && x.name !== "chips_staking"
        )
        .map((x) => ({
          name: _.upperCase(x.name),
          value:
            API.get("profitshare", "profitshareBalance", x.name) /
              Math.pow(10, x.decimals) || 0,
          price: x.price || 1,
        }))
        .value();
    ...
    bot.command("top", async (ctx) => {
      ...
      const racesRanks = await Promise.all(
        _.map(
          activeRaces,
          (race) =>
      ...
      bot.telegram.sendMessage(ctx.chat.id, models.top(racesRanks), {
        parse_mode: "HTML",
    ...
    bot.command("events", async (ctx) => {
      const activeRaces = await API.listActiveRaces((skip = 0), (limit = 10));
      bot.telegram.sendMessage(ctx.chat.id, models.events(activeRaces), {
        parse_mode: "HTML",
        disable_web_page_preview: true,
      });
    });
    bot.command("prices", (ctx) => {
      const currencies = _.chain(API.get("public", "currencies"))
        ...
        .value();
      bot.telegram.sendMessage(ctx.chat.id, models.prices(currencies), {
        parse_mode: "HTML",
      });
    });
    bot.command("groups", (ctx) => {
      bot.telegram.sendMessage(
        ctx.chat.id,
        models.groups(config.ambassadors),
        {}
      );
    });
