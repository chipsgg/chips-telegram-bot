...
command.handlers.discord = async ({ interaction, commands, createEmbed }) => {
	...
	const hCommand =
		interaction.options.getString('command', false) ?? undefined;
	...
	if (!hCommand) {
		...
		const command =
			cmd instanceof CommandGroup ? cmd.subcommands[sub] : cmd;

		if (
			!command ||
			command instanceof CommandGroup ||
			!command.isChatInputCommand()
		) {
		...
		const embed = createEmbed().setTitle(
			`Usage for /${command.displayName}`
		);
...
async function commandAutocomplete({
	...
	const searchString =
		option.value.replace(/[^a-zA-Z0-9-]/g, '') || undefined;
	...
	async function getCommandOptions(commandName: string) {
		...
		const matchIndex = (str: string) =>
			str.toLowerCase().indexOf(commandName);
		...
		const results = Array.from(commands.values())
			...
			.filter(
				(name) =>
					...
					name.toLowerCase().includes(commandName)
			)
