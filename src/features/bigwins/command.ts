...
const command = new ChipsCommand<Process>({
	name: 'bigwins',
	description: 'Rankings of the biggest wins on Chips.gg!',
	...
	process: async (ctx) => {
		...
		const bigwins = (await ctx.sdk.get('stats', 'bets', 'bigwins')) as Record<string, Win>;
		...
		const wins = Object.values(bigwins).sort((a, b) => b.bet.winningsInUsd - a.bet.winningsInUsd);
