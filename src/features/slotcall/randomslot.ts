...
export function discordEmbed({ slot }: RandomSlotProcess) {
	...
	const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
		new ButtonBuilder().setStyle(ButtonStyle.Link).setLabel('Play now!').setURL(`https://chips.gg/slots/${slot.slug}`),
		new ButtonBuilder().setStyle(ButtonStyle.Primary).setLabel('Reroll').setCustomId('SLOTCALL'),
	);
