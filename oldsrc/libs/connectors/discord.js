...
const discordMakeForm = (options) => {
	const { emoji, title, content, footer, banner, url, buttonLabel, ephemeral } = options;
	...
	if (footer) embed.setFooter(_.trim(footer), 'https://cdn.chips.gg/public/images/assets/favicon/favicon-32x32.png');
			...
			try {
				client.guilds.cache.forEach((guild) => {
					const chan = guild.channels.cache
						.filter((channel) => channel.permissionsFor(client.user).has('SEND_MESSAGES') && channel.isText())
						.first();
					if (chan) {
