...
module.exports = (api) => {
	...
	const commands = {
		...
		prices: {
			...
			handler: async (ctx) => {
				...
				let currencies = _.chain(await api.public('currencies')).filter(
					(x) =>
						!x.hidden &&
						x.name !== 'chips' &&
		...
		promotions: {
			...
			handler: async (ctx) => {
				const activeRaces = await api.public('listRunningPromotions');
			...
			handler: (ctx) => {
				...
				const top = _.chain(bigwins)
					...
					.map((obj) => {
						...
						obj.bet.amountInDollar = (obj.bet.amount / Math.pow(10, decimals)) * price;
						obj.bet.winningsInDollar = (obj.bet.winnings / Math.pow(10, decimals)) * price;
						return obj;
					})
					.value();
		...
		linkaccount: {
			...
			handler: async (ctx) => {
				...
				try {
					const account = await api.auth('linkPlatformID', payload);

					const [player, vip] = await Promise.all([
						api.public('getUser', {
							userid: account.userid,
						}),
						api.public('getUserVipRank', {
							userid: account.userid,
						}),
					]);
	...
	commands.user = {
		...
		handler: async (ctx) => {
			...
			try {
				const user = await api.public('getUser', { userid: username });
				if (!user) {
	...
	commands.mostplayed = {
		...
		handler: async (ctx) => {
			...
			return ctx.sendForm({
				...
				content: games.map((game, index) => `${index + 1}. ${game.title} (${game.provider})`).join('\n'),
	...
	commands.promotion = {
		...
		handler: async (ctx) => {
			...
			try {
				const promotion = await api.public('getPromotion', {
					// game: "promotion",
